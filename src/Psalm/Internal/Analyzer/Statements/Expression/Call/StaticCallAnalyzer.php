    ...
    public static function analyze(
        ...
        foreach ($lhs_type->getTypes() as $lhs_type_part) {
            ...
            if ($stmt->name instanceof PhpParser\Node\Identifier && !$is_mock) {
                ...
                foreach ($codebase->migrations as $original_pattern => $transformation) {
                    if (strtolower($declaring_method_id) . '\((.*\))' === $original_pattern) {
                        if (strpos($transformation, '($1)') === strlen($transformation) - 4
                            ...
                            $file_manipulations[] = new \Psalm\FileManipulation(
                                $stmt->class->getAttribute('startFilePos'),
                                $stmt->class->getAttribute('endFilePos') + 1,
                            ...
                            $file_manipulations[] = new \Psalm\FileManipulation(
                                $stmt->name->getAttribute('startFilePos'),
                                $stmt->name->getAttribute('endFilePos') + 1,
                                $new_method_name
                            );
